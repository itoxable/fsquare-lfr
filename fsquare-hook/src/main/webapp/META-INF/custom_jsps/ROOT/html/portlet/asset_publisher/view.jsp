...
<c:choose>
	<c:when test='<%= !paginationType.equals("none") && !paginationType.equals("continous") && (searchContainer.getTotal() > searchContainer.getResults().size()) %>'>
		<liferay-ui:search-paginator searchContainer="<%= searchContainer %>" type="<%= paginationType %>" />
	</c:when>
	<c:when test='<%= paginationType.equals("continous") %>'>
		...
		<aui:script use="aui-base,selector-css3,aui-io-request">
			...
			/*portletURL.setParameter("delta", searchContainer.getDelta());
			portletURL.setParameter("deltaParam", searchContainer.getDeltaParam());
			portletURL.setParameter("total", searchContainer.getTotal());
			portletURL.setParameter("curParam", searchContainer.getCurParam());
			portletURL.setParameter("cur", searchContainer.getCur());
			portletURL.setParameter("deltaConfigurable", searchContainer.isDeltaConfigurable());
			portletURL.setParameter("maxPages", PropsValues.SEARCH_CONTAINER_PAGE_ITERATOR_MAX_PAGES);
			*/

			%>

			Liferay.provide(window, '<portlet:namespace />loadMore',
				function() {
		        	A.io.request('<%=portletURL%>',{
		                  ...
		                  data: { 
		                  	...
		                  	<portlet:namespace />deltaParam: "<%= searchContainer.getDeltaParam() %>",
		                  	<portlet:namespace />total: "<%= searchContainer.getTotal() %>",
		                  	<portlet:namespace />curParam: "<%= searchContainer.getCurParam() %>",
		                  	<portlet:namespace />cur: "<%= searchContainer.getCur() %>",
		                  	<portlet:namespace />deltaConfigurable: "<%= searchContainer.isDeltaConfigurable() %>",
		                  	<portlet:namespace />maxPages: "<%=  PropsValues.SEARCH_CONTAINER_PAGE_ITERATOR_MAX_PAGES %>"
		                  },
		                  on: {
		                      success: function() {
		...
		<a href="javascript:;" onclick="<portlet:namespace />loadMore()" class="continous-pagination-button"><span>Show more</span></a>
	</c:when>
</c:choose>
